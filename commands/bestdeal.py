...
def bestdeal_parser(api_data, hotels_amount: int, photos_flag: bool, price_range: tuple, distance_range: tuple) -> list:
    ...
    for elem in api_data['data']['propertySearch']['properties']:
        ...
        if (price_range[0] <= round(elem['price']['lead']['amount']) <= price_range[1]) and (
            distance_range[0] <= round(elem['destinationInfo']['distanceFromDestination']['value'] * 1.609, 2) <= distance_range[1]):
            ...
            if len(hotels_list) != hotels_amount and round(elem['price']['lead']['amount']) != 0:
                ...
                for hotel in hotels_list[:]:
                    ...
                    if (hotel.cost < round(elem['price']['lead']['amount']) != 0
                            and hotel.distance <= elem['destinationInfo']['distanceFromDestination']['value']) or (
...
def bestdeal(params: dict, user_id) -> Union[tuple, Type[TypeError], Type[ValueError], Type[NameError]]:
    ...
    start_price, end_price = int(params['startPrice']), int(params['endPrice'])
    start_distance, end_distance = int(params['startDistance']), int(params['endDistance'])
    ...
    hotels_list = bestdeal_parser(properties_data, hotels_amount, photos_flag, (start_price, end_price), (start_distance, end_distance))
