...
def database_decorator(func: Callable) -> Callable:
    ...
    def wrapped_func(params):
        ...
        try:
            ...
            add_hotels_query = '''INSERT INTO hotels (
            command_id, hotel_id, hotel_name, hotel_price, image, distance, description, address, total_cost)
            VALUES 
            '''
            ...
            for hotel in hotels:
                if result.index(hotel) != len(result) - 1:
                    add_hotels_query += f'({command_id}, {hotel.id}, {hotel.name}, {hotel.cost}, ' \
                                        f'{hotel.image}, {hotel.distance}, {hotel.description}, ' \
                                        f'{hotel.address}, {hotel.total}), '
                else:
                    add_hotels_query += f'({command_id}, {hotel.id}, {hotel.name}, {hotel.cost}, ' \
                                        f'{hotel.image}, {hotel.distance}, {hotel.description}, ' \
                                        f'{hotel.address}, {hotel.total});'

            cursor.execute(add_hotels_query)
